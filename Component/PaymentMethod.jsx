...
export default function PaymentMethod(){
    ...
    const getData = async () => {
        const ftch = await fetch(`/api/getItems?name=TypePayment`);
    ...
    const handlerSubmit = async (e) => {
        e.preventDefault();
        const ftch = await fetch(`/api/addItems?name=TypePayment&nameRecord=${name}&price=${price}`);
        const res = await ftch.json();
        getData();
        setName("");
        setPrice("");
    ...
    return (
        <>
            {
                (showModal) ? <AddItems title="Add a new payment method detail"
