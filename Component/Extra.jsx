...
export default function Extra(){
    ...
    const getData = async () => {
        const ftch = await fetch(`/api/getItems?name=Extra`);
    ...
    const handlerSubmit = async (e) => {
        e.preventDefault();
        const ftch = await fetch(`/api/addItems?name=Extra&nameRecord=${name}&price=${price}`);
        const res = await ftch.json();
        getData();
        setName("");
        setPrice("");
    ...
    return (
        <>
            {
                (showModal) ? <AddItems title="Add a new extra detail"
